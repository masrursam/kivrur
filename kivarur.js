...
const loadTokens = async () => {
    try {
        const data = await fs.readFile('tokenrur.txt', 'utf8');
        return data.trim().split('\n').map(token => token.trim());
    } catch (error) {
        console.error('Error loading tokens from tokenrur.txt:', error.message);
        process.exit(1);
    }
};
...
const loadProxies = async () => {
    try {
        const data = await fs.readFile('proxiesrur.txt', 'utf8');
        proxyList = data.trim().split('\n').map(proxy => proxy.trim());
        console.log(`Loaded ${proxyList.length} proxies from proxiesrur.txt`);
    } catch (error) {
        console.error('Error loading proxies from proxiesrur.txt:', error.message);
        process.exit(1);
    }
};
...
async function runBot() {
    ...
    console.log(`Loaded ${tokens.length} tokens from tokenrur.txt`);
